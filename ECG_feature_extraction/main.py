...
if __name__ == '__main__':
    ...
    # write results into a file (CSV)
    file_name = m_detector.get_name()
    out_dir = "./results"
    os.makedirs(out_dir, exist_ok=True)

    csv_path = os.path.join(out_dir, file_name + ".csv")
    ...
    header = [
        ...
        "FP (Beats)",
        "FN (Beats)",
        "Failed Detection (Beats)",
        "Failed Detection (%)",
        "mean hr detection error (bpm)",
        "std hr detection error (bpm)",
    ]
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        ...
        res = [
            ...
            total_FP_db,
            total_FN_db,
            total_FP_db + total_FN_db,
            (total_FP_db + total_FN_db) / total_beats_db * 100 if total_beats_db else 0.0,
            0.0,
            0.0,
        ]
        writer.writerow(res)
        ...
        # metrics
        writer.writerow(["Sensitivity:", se])
        writer.writerow(["PPV:", ppv])
        ...
        # time
        writer.writerow(["Time Elapsed (s):", time_elapsed])
