...
if __name__ == "__main__":
    ...
    for i, dat_path in enumerate(files, 1):
        ...
        if "III" in leads: leads_to_check.append("III")
        if not leads_to_check:
            # 兩個都沒有時，fallback 到第 0 導程
            leads_to_check = [leads[LEAD_FALLBACK_IDX]]
            print(f"[{i}] 警告：{base.name} 無 Lead I/II/III，改用 {leads_to_check[0]}")
        ...
        for lead_name in leads_to_check:
            ...
            feats["HR"] = round(feats["HR"], 3)
            feats["Oldpeak"] = round(feats["Oldpeak"], 3)
            feats["OldpeakAbs"] = round(feats["OldpeakAbs"], 3)
            feats["ST_Slope"] = round(feats["ST_Slope"], 3)
